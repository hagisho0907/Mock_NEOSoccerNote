...
### 1. **Home** - 総合ダッシュボード ⭐ 大幅改善
**概要**: 全データページから収集した総合パフォーマンス指標

**主要機能**:
- **カテゴリ別KPI表示**: 14項目のデータ駆動型指標
  - ⚽ パフォーマンス指標（Stats）: パス成功率、デュエル勝率、シュート精度、平均評価
  - 🌱 ライフスタイル指標（Lifelog）: 睡眠効率、日間PFC、運動強度、消費カロリー
  - 🏥 メディカル指標（Medical）: FMSスコア、筋力バランス、可動域状況、リスク要因
  - 📈 活動指標（Session）: 週間活動、連続日数
- **その他データ**: 
  - アラート: 危険データの自動検出・警告
  - ネクストアクション: Noteページの未完了タスク表示
  - 最新動画: 直近登録動画のプレビューと再生
...
**改善内容**:
- ✅ 実データから計算可能な14項目のKPIに拡張
- ✅ カテゴリ別色分けレイアウト
- ✅ アラート・タスク・動画の追加情報セクション
